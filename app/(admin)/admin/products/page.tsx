...
export default function ProductsPage() {
  ...
  const addArrayItem = (
    field: "ingredients" | "skinTypes" | "benefits",
    value: string,
  ) => {
    if (value.trim()) {
      setFormData((prev) => ({
  ...
  const handleEdit = async (product: any) => {
    ...
    if (product.imageUrls && product.imageUrls.length > 0) {
      if (product.images && product.images.length > 0) {
        ...
        existingImages = product.images.map(
        ...
        existingImages = product.imageUrls.map(
              ...
              : products.map((product) => (
                  ...
                  >
                    <div className="aspect-square bg-gray-100 relative">
                      ...
                      {/* Edit Indicator */}
                      <div
                        className="absolute top-2 left-2 bg-green-500 text-white rounded-full w-8 h-8 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 shadow-lg"
                        title="Click to edit"
                      >
